...
export class PeerjsClient implements IPeerClient {
  ...
  /**
   * Handles the connection to peerjs server
   */
  ...
  constructor(private options: Options) {
    this.peerClient = new Peer(this.options);
    this.serverConnect$ = fromPeerEvent(this.peerClient, "open").pipe(
      tap((id) => {
        if (typeof id === "string") {
          this.state$.next({
            peerId: id,
    ...
    this.serverDisconnect$ = fromPeerEvent(
      this.peerClient,
      "disconnected"
    ).pipe(
      tap(() => {
    ...
    this.serverError$ = fromPeerEvent(this.peerClient, "error").pipe(
      tap(() => {
        // this.state$.next({ networkStatus: NetworkStatus.DISCONNECTED });
      })
    );

    this.incomingConnection$ = fromPeerEvent(
      this.peerClient,
      "connection"
    ).pipe(
      mergeMap((conn) => {
    ...
    this.events$ = this.incomingConnection$.pipe<any, PeerEvent>(
    ...
    this.requests$ = this.incomingConnection$.pipe<any, PeerRequest>(
      map((data) => {
    ...
    this.disposals.push(
      ...
      this.incomingConnection$.subscribe().unsubscribe
    );
  }
  ...
  emit(
    signal: Omit<PeerRequest, "targetId"> | PeerEvent,
